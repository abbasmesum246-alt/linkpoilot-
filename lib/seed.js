...
import { q, insertMany, now, daysAgo } from './db.js';
...
const NET_RATES = { 'Amazon Associates': 0.04, 'ClickBank': 0.45, 'ShareASale': 0.15, 'Impact': 0.20, 'PartnerStack': 0.25, 'Digistore24': 0.30 };

export async function seedDemoUser() {
  const existing = await q.get('SELECT id FROM users WHERE email = ?', 'demo@linkpilot.app');
  if (existing) {
    await q.run('UPDATE users SET is_demo = 1 WHERE id = ?', existing.id);
    return existing;
  }
  ...
  try {
    userRes = await q.run(
      ...
      'Alex Morgan', 'demo@linkpilot.app', passwordHash, 'Morgan Media', daysAgo(240)
    );
  } catch (e) {
    // concurrent cold start already seeded — reuse it
    const raced = await q.get('SELECT id FROM users WHERE email = ?', 'demo@linkpilot.app');
    if (!raced) throw e;
    await q.run('UPDATE users SET is_demo = 1 WHERE id = ?', raced.id);
    return raced;
  }
  ...
  const netIds = {};
  for (const [name, color, rate, cookie, status, notes] of networkDefs) {
    const r = await q.run(
      `INSERT INTO networks (user_id, name, color, commission_rate, cookie_days, status, notes, created_at) VALUES (?,?,?,?,?,?,?,?)`,
      uid, name, color, rate, cookie, status, notes, daysAgo(200)
    );
    netIds[name] = r.lastInsertRowid;
  }
  ...
  const campIds = {};
  for (const [name, desc, status, budget, color, start, end] of campaignDefs) {
    const r = await q.run(
      `INSERT INTO campaigns (user_id, name, description, status, budget, color, starts_at, ends_at, created_at) VALUES (?,?,?,?,?,?,?,?,?)`,
      uid, name, desc, status, budget, color, start, end, daysAgo(100)
    );
    campIds[name] = r.lastInsertRowid;
  }
  ...
  const linkIds = {};
  for (const [name, slug, dest, net, camp, , , , ,] of linkDefs) {
    const r = await q.run(
      ...
      slug === 'airpods-pro-2' ? 'Best performer. Pinned in all gift-guide posts.' : '',
      daysAgo(90), daysAgo(Math.floor(Math.random() * 5) + 1)
    );
    linkIds[slug] = r.lastInsertRowid;
  }

  // ---------- Clicks (92 days of history, batched inserts) ----------
  const rng = mulberry32(20260819);
  const linkMeta = {};
  for (const [name, slug, , , , price, cr, base, growth] of linkDefs) {
    linkMeta[slug] = { price, cr, base, growth };
  }
  const CLICK_COLS = ['link_id', 'user_id', 'referrer', 'country', 'device', 'converted', 'revenue', 'created_at'];
  const DAYS = 92;
  const allRows = [];
  for (let d = DAYS; d >= 0; d--) {
    ...
    for (const [name, slug, , , , , , ,] of linkDefs) {
      ...
      for (let c = 0; c < count; c++) {
        ...
        const net = linkDefs.find(l => l[1] === slug)[3];
        const rate = NET_RATES[net] || 0.1;
        const qty = rng() < 0.09 ? 2 : 1;
        const revenue = converted ? +(price * rate * qty).toFixed(2) : 0;
        allRows.push([linkIds[slug], uid, weightedPick(rng, REFERRERS), weightedPick(rng, COUNTRIES), weightedPick(rng, DEVICES), converted, revenue, ts.toISOString()]);
      }
    }
  }
  await insertMany('clicks', CLICK_COLS, allRows);
  ...
  for (const [net, amount, status, method, ref, notes, req, paid] of payoutDefs) {
    await q.run(
      `INSERT INTO payouts (user_id, network_id, amount, status, method, reference, notes, requested_at, paid_at, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)`,
      uid, netIds[net], amount, status, method, ref, notes, daysAgo(req), paid ? daysAgo(paid) : null, daysAgo(req)
    );
  }
  ...
  await q.run(
    ...
    uid, 'Production key',
    `lpk_${Array.from({ length: 32 }, () => 'abcdefghijklmnopqrstuvwxyz0123456789'[Math.floor(Math.random() * 36)]).join('')}`,
    daysAgo(60), daysAgo(1)
  );
  ...
  for (const [type, msg, days] of activityDefs) {
    await q.run('INSERT INTO activity (user_id, type, message, created_at) VALUES (?,?,?,?)', uid, type, msg, daysAgo(days));
  }

  // ---------- Settings row ----------
  await q.run(
    `INSERT INTO settings (user_id, webhook_url, webhook_secret, webhook_events, created_at) VALUES (?,?,?,?,?)`,
    uid, '', '', 'click,conversion,payout', daysAgo(240)
  );

  console.log(`[seed] demo user created with ${DAYS} days of click history (${allRows.length} clicks)`);
  return { id: uid };
}
