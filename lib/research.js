...
async function cacheGet(key) {
  const row = await q.get('SELECT data, fetched_at FROM research_cache WHERE key = ?', key);
  if (!row) return null;
...
async function cacheSet(key, data) {
  await q.run('INSERT INTO research_cache (key, data, fetched_at) VALUES (?,?,?) ON CONFLICT(key) DO UPDATE SET data=excluded.data, fetched_at=excluded.fetched_at',
