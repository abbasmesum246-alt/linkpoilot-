import { q, insertMany } from './db.js';
...
export async function seedMarket() {
  const typeCount = await q.get('SELECT COUNT(*) n FROM affiliate_types');
  if (typeCount && +typeCount.n > 0) return;
  try {
    await insertMany('affiliate_types',
      ['slug', 'name', 'icon', 'tagline', 'description', 'avg_commission', 'best_channels', 'features', 'tips', 'sort'],
      TYPES.map((t, i) => [t.slug, t.name, t.icon, t.tagline, t.description, t.avg_commission, t.best_channels, JSON.stringify(t.features), JSON.stringify(t.tips), i]));
    await insertMany('programs',
      ['type_slug', 'name', 'network', 'commission_type', 'rate_min', 'rate_max', 'rate_label', 'cookie_days', 'payout_method', 'min_payout', 'approval', 'epc', 'growth', 'popularity', 'url', 'blurb', 'promo', 'pros', 'cons', 'best_for', 'verified'],
      PROGRAMS.map(p => [p.type, p.name, p.network, p.commission_type, p.rate_min, p.rate_max, p.rate_label, p.cookie_days, p.payout_method, p.min_payout, p.approval, p.epc, p.growth, p.popularity, p.url, p.blurb, p.promo, JSON.stringify(p.pros), JSON.stringify(p.cons), JSON.stringify(p.best_for), 0]));
  } catch (e) {
    // concurrent cold start already seeded — verify and continue
    const raced = await q.get('SELECT COUNT(*) n FROM affiliate_types');
    if (!raced || +raced.n === 0) throw e;
  }
