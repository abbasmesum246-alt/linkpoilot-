import { q, now, daysAgo } from './db.js';
...
const progByName = async (name) => {
  const clean = name.toLowerCase().replace(/[^a-z0-9]+/g, '');
  const all = await q.all('SELECT p.*, t.name AS type_name, t.icon AS type_icon FROM programs p JOIN affiliate_types t ON t.slug = p.type_slug');
...
const typeByName = async (name) => {
  return await q.get('SELECT * FROM affiliate_types WHERE lower(name) LIKE ? OR lower(tagline) LIKE ?', `%${name.toLowerCase()}%`, `%${name.toLowerCase()}%`);
};
...
// ------------------------------------------------------------ strategy generator
async function strategyFor(program, userData) {
  const type = await q.get('SELECT * FROM affiliate_types WHERE slug = ?', program.type_slug);
  ...
  const content = {
    ...
    content_ideas: [
      ...
      `${program.name} vs ${await competitorOf(program)}: which is actually better?`,
...
async function competitorOf(program) {
  const peers = await q.get('SELECT name FROM programs WHERE type_slug = ? AND name != ? ORDER BY popularity DESC LIMIT 1', program.type_slug, program.name);
...
// ------------------------------------------------------------ top programs
async function topPrograms(slug, sort = 'opportunity', limit = 6) {
  const where = slug ? 'WHERE p.type_slug = ?' : '';
  const params = slug ? [slug] : [];
  const order = sort === 'epc' ? 'p.epc DESC' : sort === 'growth' ? 'p.growth DESC' : '(p.epc * 0.5 + p.growth * 0.3 + p.popularity * 0.2) DESC';
  return await q.all(`SELECT p.*, t.name AS type_name FROM programs p JOIN affiliate_types t ON t.slug = p.type_slug ${where} ORDER BY ${order} LIMIT ?`, ...params, limit);
}
...
export async function assistantReply(userId, message, opts = {}) {
  ...
  const user = await q.get('SELECT id, name, is_demo FROM users WHERE id = ?', userId);
  ...
  if (stratMatch) {
    ...
    const prog = await progByName(target) || await progByName(target.replace(/\b(the|a|an)\b/g, '').trim());
    if (prog) {
      const s = await strategyFor(prog, userData);
      const parts = [];
      ...
      // persist strategy
      await q.run('INSERT INTO strategies (user_id, program_id, title, content, created_at) VALUES (?,?,?,?,?)',
        userId, prog.id, `${prog.name} — promotion strategy`, JSON.stringify(s), now());
      if (live) {
        try {
    ...
    if (typeSlug) {
      const type = await q.get('SELECT * FROM affiliate_types WHERE slug = ?', typeSlug);
      const tops = await topPrograms(typeSlug, 'opportunity', 3);
  ...
  if (cmp) {
    const a = await progByName(cmp[2].trim()), b = await progByName(cmp[4].trim());
    if (a && b) {
  ...
  if (/(best|top|offer|opportunit|program|earn|money|niche|high paying|highest)/.test(m)) {
    ...
    const tops = await topPrograms(typeSlug, sort, 6);
    if (!tops.length) { say('I couldn\'t find matching offers — try naming a niche like "saas", "vpn" or "finance".'); return reply; }
    const type = typeSlug ? await q.get('SELECT * FROM affiliate_types WHERE slug = ?', typeSlug) : null;
  ...
  const knownProg = await progByName(m.replace(/^(what is|tell me about|info on|details (about|of|on)|rate(s)? (for|of)|commission (for|of)|how much does|is)\s+/i, ''));
  if (knownProg) {
    const p = knownProg;
    const parts = [];
    parts.push(`# ${p.name}\n`);
  ...
  if (/(types|categories|groups|kinds)/.test(m) && /affiliate|offer|program|niche/.test(m)) {
    const types = await q.all('SELECT * FROM affiliate_types ORDER BY sort');
...
async function getUserContext(userId) {
  ...
  const overview = await q.get(`SELECT COUNT(*) clicks, COALESCE(SUM(converted),0) conversions, COALESCE(SUM(revenue),0) revenue FROM clicks WHERE user_id = ? AND created_at >= ?`, userId, since);
  const prev = await q.get(`SELECT COALESCE(SUM(revenue),0) revenue FROM clicks WHERE user_id = ? AND created_at >= ? AND created_at < ?`, userId, daysAgo(60), since);
  const cr = overview.clicks ? (overview.conversions / overview.clicks) * 100 : 0;
  const topLinks = await q.all(`
    ...
    WHERE l.user_id = ? GROUP BY l.id ORDER BY clicks_recent DESC LIMIT 5`, since, userId);
  const topReferrers = await q.all(`SELECT referrer, COUNT(*) clicks FROM clicks WHERE user_id = ? AND created_at >= ? GROUP BY referrer ORDER BY clicks DESC LIMIT 3`, userId, since);
  const pending = await q.get(`SELECT COALESCE(SUM(amount),0) s FROM payouts WHERE user_id = ? AND status != 'paid'`, userId);
  return {
    overview: {
...
async function tryLlm(userId, message, userData) {
  try {
    const s = await q.get('SELECT ai_key, ai_model, ai_provider FROM settings WHERE user_id = ?', userId);
